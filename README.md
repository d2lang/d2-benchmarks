...
Time to start the CLI, render a diagram, write the output, and exit—including browser or JVM startup. **Bold is the lowest valid time.** Each result uses 20 runs after 3 warm-ups. Basic rows are medians; real-world rows are geometric means of per-diagram medians.
...
Mermaid's TPMJS PNG omits content at 2×, so its PNG average is unavailable.
