...
The pinned public D2 binary includes both Dagre and TALA. Setup registers `d2-dagre` and `d2-tala` against that same executable, with `--layout tala --tala-seeds 1,2,3` for TALA. The fixed seeds keep its search inputs consistent across invocations.

To compare the two built-in layouts:

```sh
./make.sh --tools d2-dagre d2-tala
```

For a custom D2 build, use `kind: "d2"` with the appropriate layout arguments and record the exact source revision and build flags. Reuse the same executable for both configurations to isolate the layout choice.
