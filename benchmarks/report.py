...
def _asset(root: Path, job: dict, records: list[dict]) -> dict:
    ...
    result.update(bytes=len(data), gzip9_bytes=len(gzip.compress(data, compresslevel=9, mtime=0)),
                  sha256=hashlib.sha256(data).hexdigest())
    last = records[-1] if records else {}
    image = last.get("image") or {}
    result["verified"] = bool(last.get("success") and image.get("sha256") == result["sha256"]
                              and image.get("bytes") == result["bytes"])
    if result["verified"]:
        for key in ("width", "height", "viewBox", "width_css_px", "height_css_px", "pixels"):
            if key in image:
...
def _summarize(root: Path, run: dict, rows: list[dict], parse_errors: list[str], baseline: str, bootstrap: int) -> dict:
    ...
    for field in ("fatal_error", "input_integrity_errors", "tool_integrity_errors"):
        if run.get(field):
        ...
        for tool in tools:
            for fmt in formats:
                ...
                case = {"id": job["id"], "fixture": key[0], "title": fixture.get("title", key[0]), "tool": tool,
                        ...
                        "group": "svg" if fmt == "svg" else "png-primary" if primary else "png-supplemental",
                ...
                cases.append(case)
    complete_status = str(run.get("status", "incomplete")).lower() in {"complete", "completed", "success"}
    ...
    groups = []
    for group_name in ("svg", "png-primary", "png-supplemental"):
        ...
        if group_name != "svg" and len(densities) != 1: reasons.append("PNG density differs within this group")
        if group_name == "png-primary" and densities != {2, 2.0}: reasons.append("Primary PNG is not consistently rendered at 2x density")
        aggregates = []
        if not reasons:
            ...
            for tool in tools:
                ...
                aggregates.append({"tool": tool, "fixtures": group_fixtures,
                                   ...
                                   "raw_bytes": sum(c["asset"]["bytes"] for c in selected),
                                   "gzip9_bytes": sum(c["asset"]["gzip9_bytes"] for c in selected),
                                   "pixels": sum(c["asset"].get("pixels",0) for c in selected) if group_name != "svg" else None})
        groups.append({"id": group_name, "fixtures": group_fixtures, "fixture_count": len(group_fixtures),
...
def _write_csv(root: Path, summary: dict) -> None:
    fields = ["fixture","tool","format","raster_density","primary","eligible","expected_measured","measured_attempts","successes","failures","missing_measurements","median_ms","ci95_low_ms","ci95_high_ms","min_ms","q25_ms","q75_ms","p95_ms","max_ms","raw_bytes","gzip9_bytes","width","height","width_css_px","height_css_px","viewBox","pixels","measured_output_hash_count","output_stable","output"]
    with (root/"summary.csv").open("w",newline="") as stream:
        writer=csv.DictWriter(stream,fieldnames=fields); writer.writeheader()
        for case in summary["cases"]:
            row={key:case.get(key) for key in fields}; stats=case["timing_ms"]; asset=case["asset"]
            row.update(missing_measurements=len(case["missing_rounds"]),median_ms=stats["median"],ci95_low_ms=stats["median_ci95"][0] if stats["median_ci95"] else None,ci95_high_ms=stats["median_ci95"][1] if stats["median_ci95"] else None)
            for key in ("min","q25","q75","p95","max"): row[key+"_ms"]=stats[key]
            row.update(raw_bytes=asset.get("bytes"),gzip9_bytes=asset.get("gzip9_bytes"),width=asset.get("width"),height=asset.get("height"),width_css_px=asset.get("width_css_px"),height_css_px=asset.get("height_css_px"),viewBox=asset.get("viewBox"),pixels=asset.get("pixels"))
            writer.writerow(row)


def _write_markdown(root: Path, summary: dict) -> None:
    c=summary["completeness"]
    lines=["# Diagram CLI benchmark", "", f"Run `{summary['run_id']}` · status **{summary['status']}**", "",
           f"{c['successes']} successful measured attempts / {c['expected_measured']} expected; {c['failures']} failures, {c['missing_measurements']} missing measurements. {c['warmup_attempts']} excluded warm-ups ({c['warmup_failures']} failed).", "",
    ...
    if summary["smoke"] or summary["short_run"]: lines += ["**Smoke or short run: use this report to check execution and outputs. Aggregate rankings are suppressed.**", ""]
    lines += [f"{c['jobs_with_changing_output_hashes']} jobs produced more than one measured output hash. Changing bytes can reflect layout, identifiers or metadata; hashes alone do not explain the cause. Asset sizes describe the verified retained final output, not an average over changing outputs.", ""]
    if summary["issues"]: lines += ["## Recording issues", ""] + ["- "+_md(i) for i in summary["issues"]] + [""]
    ...
    for group in summary["groups"]:
        lines += [f"## {group['id']}", "", f"{group['fixture_count']} fixed fixtures; {group['successes']} successes / {group['expected_measurements']} expected measurements; {group['failures']} failures.", ""]
        ...
        for a in sorted(group["aggregates"],key=lambda a:a["geomean_median_ms"]):
            lines.append(f"| {_md(TOOL_NAMES.get(a['tool'],a['tool']))} | {_num(a['geomean_median_ms'])} | {_num(a['baseline_over_tool_ratio'])} | {_interval(a['ratio_ci95'])} | {a['raw_bytes']:,} | {a['gzip9_bytes']:,} | {_num(a['pixels'],0)} |")
        lines.append("")
...
<p>Whole CLI latency, output sizes and actual rendered diagrams. Every recorded failure and missing measurement remains visible. Different layouts, fonts and canvases make these equivalent semantic graphs, not identical rendering workloads.</p>
...
<div class="table-wrap"><table><thead><tr><th>Tool</th><th>Success / expected</th><th>Failed / missing</th><th>Median (ms)</th><th>95% interval (ms)</th><th>All successful samples</th><th>Min–max (ms)</th><th>Raw bytes</th><th>Gzip-9 bytes</th><th>Canvas</th><th>Output hashes</th></tr></thead><tbody id="jobs"></tbody></table></div>
<p class="note">Dots show every successful measured attempt; the vertical mark is the median. Each row uses its own min–max scale. Warm-ups are excluded. Output hash counts cover measured successes; changing hashes may reflect layout, identifiers or metadata and do not automatically invalidate timings. Raw and gzip-9 sizes describe the retained final file; gzip is most useful for SVG because PNG is already compressed. A row with failures or missing measurements has a conditional descriptive median and is excluded from aggregate rankings.</p>
<div class="pair">
...
</div><details id="failures"><summary>Failures, missing measurements and recording issues</summary><pre id="failure-detail"></pre></details>
<details open><summary>Method and interpretation</summary><p id="bootstrap-note"></p><p>Rounds are resampled jointly and the corpus is held fixed; the report does not bootstrap diagrams. Confidence intervals cannot account for uncontrolled background activity, the choice of examples, different hardware or another benchmark session. No outliers are discarded. A baseline ratio above 1 means the selected tool is faster than that baseline.</p><p>SVG, primary 2× PNG and supplemental PNG are separate groups. PNG sizes and timings depend on the resulting pixel count. Native SVG fonts, embedded resources and metadata differ across renderers. Times include startup, parsing, layout, rendering and output creation, including Java or browser startup where used.</p><p>Aggregate rankings require a completed non-smoke run, at least ten measurements and one warm-up, every planned measurement in the fixed comparison set to succeed, and all retained assets to match their final records. Incomplete tools never disappear into a success-only leaderboard.</p></details>
...
const groups={'svg':'SVG · full fixed corpus','png-primary':'PNG · primary 2× corpus','png-supplemental':'PNG · separate supplement'};
...
for(const group of D.groups){
 const section=document.createElement('section');section.append(text('h2',groups[group.id]||group.id),text('p',`${group.fixture_count} fixed fixtures · ${group.successes}/${group.expected_measurements} successful measurements · ${group.failures} failed.`, 'muted'));
 if(!group.ranking_available){section.append(text('p','Ranking unavailable: '+group.ranking_suppression_reasons.join('; '),'aggregate-note'));}
  ...
  for(const label of ['Tool','Geomean median (ms)','Baseline / tool','95% ratio interval','Raw bytes','Gzip-9 bytes','PNG pixels'])hr.append(text('th',label));head.append(hr);table.append(head);const body=document.createElement('tbody');
  for(const a of [...group.aggregates].sort((a,b)=>a.geomean_median_ms-b.geomean_median_ms)){const tr=document.createElement('tr');[name(a.tool),n(a.geomean_median_ms),n(a.baseline_over_tool_ratio),ci(a.ratio_ci95),n(a.raw_bytes),n(a.gzip9_bytes),n(a.pixels)].forEach(v=>cell(tr,v));body.append(tr);}table.append(body);wrap.append(table);section.append(wrap);
...
function panel(side){const c=current($(side).value),view=$(side+'-view'),links=$(side+'-links');view.replaceChildren();links.replaceChildren();if(!c){$(side+'-info').textContent='Job missing';view.append(text('p','No planned job','unavailable'));return;}
 $(side+'-info').textContent=`${n(c.timing_ms.median)} ms median · ${c.successes}/${c.expected_measured} successful measurements · ${n(c.asset.bytes)} bytes · ${dims(c)}`;
